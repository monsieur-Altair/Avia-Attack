...
namespace Pool_And_Particles
{
    ...
    {
        return Get(prefab, out bool unused, position, rotation, localPosition, localRotation, localScale, parent, data);
    }
        ...
        if (result == null)
        {
            if (isReusable)
            {
                foreach (var pair in _busyObjects)
                {
                    ...
                    {
                        return Get(prefab, out bool dummy, position, rotation, localPosition, localRotation, localScale,
                            parent, data);
                    }
                }
            }
